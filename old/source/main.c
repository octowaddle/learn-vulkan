...
int main() {
    ...
    result = vkCreateBuffer(device, &staging_buffer_create_info, NULL, &staging_buffer);
    ...
    vkGetBufferMemoryRequirements(device, staging_buffer, &staging_buffer_memory_requirements);

    uint32_t staging_buffer_memory_type_index = 0;
    uint8_t staging_buffer_memory_type_found = 0;
    ...
    for (uint32_t memory_properties_type_index = 0U; memory_properties_type_index < physical_device_memory_properties.memoryTypeCount; ++memory_properties_type_index) {
        if ((staging_buffer_memory_requirements.memoryTypeBits & (1 << memory_properties_type_index)) && (physical_device_memory_properties.memoryTypes[memory_properties_type_index].propertyFlags & (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)) == (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)) {
            staging_buffer_memory_type_index = memory_properties_type_index;
            staging_buffer_memory_type_found = 1;
            break;
        }
    }

    if (!staging_buffer_memory_type_found) {
        fprintf(stderr, "error: failed to find required memory type\n");
        exit(1);
    }

    const VkMemoryAllocateInfo staging_buffer_memory_allocate_info = {
    ...
    result = vkAllocateMemory(device, &staging_buffer_memory_allocate_info, NULL, &staging_buffer_device_memory);
    ...
    vkBindBufferMemory(device, staging_buffer, staging_buffer_device_memory, 0);

    void *staging_buffer_memory_map_address = NULL;
    vkMapMemory(device, staging_buffer_device_memory, 0, staging_buffer_create_info.size, 0, &staging_buffer_memory_map_address);
    memcpy(staging_buffer_memory_map_address, vertex_data, staging_buffer_create_info.size);
    ...
    uint8_t vertex_buffer_memory_type_found = 0;

    for (uint32_t memory_properties_type_index = 0U; memory_properties_type_index < physical_device_memory_properties.memoryTypeCount; ++memory_properties_type_index) {
        if ((vertex_buffer_memory_requirements.memoryTypeBits & (1 << memory_properties_type_index)) && (physical_device_memory_properties.memoryTypes[memory_properties_type_index].propertyFlags & (VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)) == (VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)) {
            vertex_buffer_memory_type_index = memory_properties_type_index;
            vertex_buffer_memory_type_found = 1;
            break;
        }
    }

    if (!vertex_buffer_memory_type_found) {
        fprintf(stderr, "error: failed to find required memory type\n");
        exit(1);
    }
    ...
    const VkCommandBufferAllocateInfo staging_to_vertex_buffer_allocate_info = {
    ...
    VkCommandBuffer staging_to_vertex_command_buffer;
    result = vkAllocateCommandBuffers(device, &staging_to_vertex_buffer_allocate_info, &staging_to_vertex_command_buffer);
    ...
    VkCommandBufferBeginInfo staging_to_vertex_command_buffer_beging_info = {
    ...
    result = vkBeginCommandBuffer(staging_to_vertex_command_buffer, &staging_to_vertex_command_buffer_beging_info);
    ...
    const VkBufferCopy staging_to_vertex_buffer_copy = {
        .srcOffset = 0,
        .dstOffset = 0,
        .size = staging_buffer_create_info.size,
    };

    vkCmdCopyBuffer(staging_to_vertex_command_buffer, staging_buffer, vertex_buffer, 1, &staging_to_vertex_buffer_copy);

    result = vkEndCommandBuffer(staging_to_vertex_command_buffer);
    ...
    const VkSubmitInfo staging_to_vertex_submit_info = {
        ...
        .commandBufferCount = 1,
        .pCommandBuffers = &staging_to_vertex_command_buffer,
    ...
    vkFreeCommandBuffers(device, command_pool, 1, &staging_to_vertex_command_buffer);

    vkDestroyBuffer(device, staging_buffer, NULL);
    vkFreeMemory(device, staging_buffer_device_memory, NULL);
